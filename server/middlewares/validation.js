...
const validateTransaction = (req, res, next) => {
  ...
  if (!validTypes.includes(type)) {
    return res
      .status(400)
      .json({ error: "Type must be income, expense, or transfer" });
  }
...
const validateSubscription = (req, res, next) => {
  const { name, amount, category, startDate, billingCycle, nextBillingDate } =
    req.body;
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "Name, amount, category, start date, billing cycle, and next billing date are required",
    });
  }
  ...
  if (!validCycles.includes(billingCycle)) {
    return res.status(400).json({
      error:
        "Billing cycle must be weekly, monthly, quarterly, biannual, or annual",
    });
  }
...
const validateBudget = (req, res, next) => {
  const { name, amount, category, startDate, endDate, resetPeriod } = req.body;

  if (!name || !amount || !category || !startDate || !endDate || !resetPeriod) {
    return res.status(400).json({
      error:
        "Name, amount, category, start date, end date, and reset period are required",
    });
  }
...
const validateGoal = (req, res, next) => {
  ...
  if (isNaN(parseFloat(targetAmount))) {
    return res
      .status(400)
      .json({ error: "Target amount must be a valid number" });
  }
  ...
  if (!dateRegex.test(targetDate)) {
    return res
      .status(400)
      .json({ error: "Target date must be in YYYY-MM-DD format" });
  }
